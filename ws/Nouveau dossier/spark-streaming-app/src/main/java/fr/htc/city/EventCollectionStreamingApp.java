...
//        JavaPairDStream<String, Integer> events = 
        		lines.map(lineEvent -> Event.parse(lineEvent))        		
        		.mapToPair(objEvent ->  new Tuple2<String, Integer>(objEvent.getCity(), objEvent.getValue()))
        		.foreachRDD(rdd -> {
        			for (Tuple2<String, Integer> tuple : rdd.collect()) {
						System.out.println("Ville : " + tuple._1() + " | Température : " + tuple._2());
					}
