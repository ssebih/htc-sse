...
LINES TERMINATED BY '\n'
STORED AS TEXTFILE
LOCATION '/user/raj_ops/sse/hive2/sales.raw'
 ...
 ***********************
 --5.2	Création d'une table partitionned and clustred
...
unit_sales String)
Partitioned by (store_id String)
clustred by (product_id String)
ROW FORMAT DELIMITED
FIELDS TERMINATED BY ';'
LINES TERMINATED BY '\n'
STORED AS CSVFILE
LOCATION '/user/raj_ops/sse/hive/partitionned.sales'
...
--6 créer une table d'objets :
create table person (name string, friends array<string>, phones 
map<string,bigint>, details struct<job:string, salary:int, age:int, 
married:string>) row format delimited fields terminated by ';' Collection ITEMS 
terminated by ',' map keys terminated by ':' lines terminated by '\n'


--créer une table interne avec un format de fichier prédéfini :
